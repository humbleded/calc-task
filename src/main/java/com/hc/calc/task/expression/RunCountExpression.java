 ...
 */
@Component("runCount")
public class RunCountExpression extends BaseExpression {
    ...
    public ExecuteRes execute(ComputingUnit unit) throws Exception {
        ...
        list = dataService.getAllDatasNew(unit.getCalcParms().get(0).getMpointid(),
                ...
                unit.getCalcParms().get(0).getDatasource(), unit.getStartDT().getTime(), end);
        ...
        for (int i = 0; i < list.size(); i++) {
            if (0 == lastValue && 1 == list.get(i).getValue()) {
                result = result + 1;
            }
            lastValue = (int) list.get(i).getValue();
        }
        logger.info("任务id:{}计算{}时的数据，在源测点id:{}从{}到{}时间范围内共取到数据{}条,启动次数{}",
