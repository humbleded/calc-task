 ...
 */
@Component("avg")
public class AvgExpression extends BaseExpression {
	...
	public ExecuteRes execute(ComputingUnit unit) throws Exception {
		logger.info("平均值算法参数:"+unit.toString());
		ExecuteRes excuteRes = new ExecuteRes();
		Double result;
		result = dataService.avg(unit.getCalcParms().get(0).getMpointid(), unit.getCalcParms().get(0).getPoint(),
				unit.getCalcParms().get(0).getDatasource(), unit.getStartDT(), unit.getEndDT());
		logger.info("任务id:{}计算{}时的数据，在源测点id:{}从{}到{}时间范围内平均值为{}", unit.getTaskid(), DateUtil.format(unit.getSaveDT()),
